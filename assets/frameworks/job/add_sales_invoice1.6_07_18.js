...
//    $("#panel_quarter").hide();
    ...
    if(customer!=''){
        ...
        total_due_amount = 0;
        total_over_payment = 0;
        loadSAlesInvoiceCusomerPayment(customer);
    }
    ...
    function setGridTableData(resultData) {
        ...
        $.each(orderDetails, function(index, item) {
            ...
            var row = `<tr serial_batch='0' ri="${index + 1}" id="${index + 1}" proCode="${item.productCode}" uc="${item.unitOrCase}" proname = "${item.productName}"
                            qty="${item.saleQuantity}" freeqty="${item.SalesFreeQty}" unit_price="${item.unitPrice}" netamount="${item.totalAmount}" netAmount="${item.totalNetAmount}" 
